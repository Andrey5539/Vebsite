...
// ==========================================
let productsFromXML = [];      // Массив продуктов из XML
...
function loadProductsFromXML() {
    return fetch('data.xml')
        .then(response => {
            if (!response.ok) {
                throw new Error('Ошибка загрузки XML файла');
            }
        ...
        .then(xmlString => {
            ...
            productNodes.forEach(node => {
                const product = {
                    ...
                    popular: node.getAttribute('popular') === 'true',
                    name: node.querySelector('name')?.textContent || '',
                    description: node.querySelector('description')?.textContent || '',
                    price: parseFloat(node.querySelector('price')?.textContent) || 0,
                    coverage: parseFloat(node.querySelector('coverage')?.textContent) || 0,
                    image: node.querySelector('image')?.textContent || 'img/placeholder.jpg',
                    features: []
                };
                
                node.querySelectorAll('feature').forEach(feature => {
                    product.features.push(feature.textContent);
                });
                
                products.push(product);
            });
            
            productsFromXML = products;
            return products;
        })
        .catch(error => {
...
function applyFilters() {
    const filtered = productsFromXML.filter(product => {
...
// ==========================================
// ЗАПОЛНЕНИЕ SELECT (КАЛЬКУЛЯТОР И APPLY)
// ==========================================
...
function initApplyPage() {
    ...
    if (productId) {
        const product = productsFromXML.find(p => p.id === productId);
        if (product) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadProductsFromXML().then(() => {
        ...
        if (mainContent) {
            mainContent.innerHTML = '<div class="error">Ошибка загрузки данных. Пожалуйста, обновите страницу.</div>';
        }
    });
});